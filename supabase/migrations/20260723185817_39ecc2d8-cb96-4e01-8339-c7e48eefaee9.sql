...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.assistidos WHERE user_id = NEW.user_id) THEN
    ...
    ELSE
      INSERT INTO public.assistidos (nome, cpf, celular, email, status, user_id, origem_cadastro)
      VALUES (
...
BEGIN
  ...
  RAISE NOTICE 'Backfill e-mail assistidos: % linhas atualizadas', v_afetados;
END $$;
