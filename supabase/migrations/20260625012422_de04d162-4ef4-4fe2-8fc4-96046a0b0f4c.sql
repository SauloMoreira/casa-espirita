...
BEGIN
  ...
  -- Correção L-07 / AVM-001: v_evento é text; o 1º parâmetro de fn_enqueue_notificacao
  -- é o enum notif_evento (text -> enum não tem cast implícito na resolução de
  -- função). Sem o cast explícito, qualquer registro de presença "presente"/
  -- "ausente" falhava em runtime. O 3º parâmetro (template_codigo) é text.
