...
-- =========================================================
-- Etapa 2 — Gestão de Acesso unificada e setorizada
-- Concessão/revogação direta e auditada de acessos OPERACIONAIS
-- (assistido = papel base automático, nunca gerenciado aqui)
-- (admin/administrador_master = fluxo de aprovação reforçado, inalterado)
-- =========================================================
...
CREATE OR REPLACE FUNCTION public.fn_revogar_acesso_operacional(
  p_target_user_id uuid,
  p_role app_role,
  p_motivo text DEFAULT NULL
)
...
GRANT EXECUTE ON FUNCTION public.fn_conceder_acesso_operacional(uuid, app_role, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_revogar_acesso_operacional(uuid, app_role, text) TO authenticated;
